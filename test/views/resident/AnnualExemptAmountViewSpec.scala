...
class AnnualExemptAmountViewSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper {
  "The Annual Exempt Amount view" should {
    lazy val postAction = controllers.resident.shares.routes.DeductionsController.submitAnnualExemptAmount()
    lazy val backLink = Some(controllers.resident.shares.routes.DeductionsController.lossesBroughtForward().toString)
    ...
    "have a back button that" should {
      ...
      "have a link to Brought Forward Losses" in {
        backLink.attr("href") shouldBe controllers.resident.shares.routes.DeductionsController.lossesBroughtForward().toString
      }
    }
    ...
    "have a form" which {
      ...
      s"has the action '${controllers.resident.shares.routes.DeductionsController.submitAnnualExemptAmount().toString}'" in {
        form.attr("action") shouldBe controllers.resident.shares.routes.DeductionsController.submitAnnualExemptAmount().toString
      }
